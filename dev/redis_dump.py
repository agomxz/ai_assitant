...
def connect_to_redis():
    try:
        r = redis.Redis(host=HOST, port=PORT, db=DB, password=PASSWORD, decode_responses=DECODE)
        r.ping()
...
def pretty_print_value(r, key):
    ...
    if type_str == 'string':
        ...
        print(f"    First Entry ID: {info.get('first-entry', [None, {}])[0] if info.get('first-entry') else 'N/A'}")
        print(f"    Last Entry ID: {info.get('last-entry', [None, {}])[0] if info.get('last-entry') else 'N/A'}")
        ...
        print(f"  Warning: Unsupported or unknown type: {type_str} (raw dump not shown)\n")
